...
const Quiz = () => {
  ...
  useEffect(() => {
    const savedModuleTitle = localStorage.getItem('quizModuleTitle');
    const savedProgramTitle = localStorage.getItem('quizProgramTitle');
    const savedProgramId = localStorage.getItem('quizProgramId');
    const savedModuleId = localStorage.getItem('quizModuleId');

    if (!savedModuleTitle || !savedProgramTitle || !savedProgramId || !savedModuleId) {
      toast({
        title: 'Quiz Not Available',
        description: 'Please start the quiz from a module.',
        status: 'warning',
        duration: 3000,
        isClosable: true,
      });
  ...
  const fetchQuizQuestions = async (progId: string, modId: string) => {
    try {
      const response = await axios.get(`http://localhost:3001/api/quiz-questions/module/${progId}/${modId}`);
    ...
    <Box bg="gray.50" minH="100vh">
      ...
      <Box>
        ...
        >
          ...
          {timeElapsed > 0 && (
            <Box mb={6}>
              <Heading size="md" color="#1e2738" mb={2}>{moduleTitle}</Heading>
